...
export function HealthCompact() {
  ...
  const tiles = [
    { icon: <Dumbbell className="size-3.5" />, label: "WORKOUT", line1: "Push Day", line2: `${workoutDone}/${workoutExercises.length} done`, active: true },
    { icon: <UtensilsCrossed className="size-3.5" />, label: "MEALS", line1: "Tikka Masala", line2: "40m prep" },
    { icon: <ShoppingCart className="size-3.5" />, label: "GROCERY", line1: `${groceryItems.length} items`, line2: `${groceryChecked} checked` },
    {
      ...
      line2: (
        <span className="flex items-center gap-0.5">
          <Flame className="size-2.5 text-accent" />5 streak
        </span>
      ),
    },
  ]
    ...
    <div className="grid h-full grid-cols-4 gap-1.5">
      {tiles.map((t, i) => (
        <div
          key={i}
          className={cn(
            "flex cursor-pointer flex-col items-center justify-center gap-0.5 border border-border/20 bg-muted/5 px-1 py-1 text-center transition-colors hover:border-primary/30 hover:bg-primary/[0.03]",
            t.active && "border-primary/40 bg-primary/[0.03]"
          )}
        >
          <span className="text-primary/80">{t.icon}</span>
          <span className="font-display text-[7px] tracking-widest text-primary/70">{t.label}</span>
          <span className="font-mono text-[10px] leading-tight text-foreground/90">{t.line1}</span>
          <span className="font-mono text-[9px] leading-tight text-muted-foreground/60">{t.line2}</span>
        </div>
      ))}
    </div>
  )
...
export function HealthExpanded() {
  return (
    <Tabs defaultValue="workout" className="gap-2">
      <TabsList className="h-7 w-full bg-muted/20">
      ...
      <TabsContent value="workout" className="max-h-64 overflow-y-auto">
        <div className="mb-2 flex items-center justify-between font-mono text-[10px]">
          ...
          <span className="text-muted-foreground">
            {workoutExercises.filter((e) => e.done).length}/{workoutExercises.length} complete
          </span>
        </div>
          ...
          {workoutExercises.map((ex, i) => (
            <div
              ...
              className={cn(
                ...
                ex.done ? "border-l-green-500/40 text-muted-foreground" : "border-l-primary/20 text-foreground/90"
              )}
            >
              <span
                className={cn(
                  "size-3 shrink-0 rounded-sm border",
                  ex.done ? "border-green-500 bg-green-500/20" : "border-muted-foreground/30"
                )}
              />
        ...
        <div className="space-y-1">
          {todayMeals.map((m, i) => (
            <div
              ...
              className={cn(
                "flex items-center gap-2 border-l-2 py-1 pl-2 font-mono text-xs",
                m.done ? "border-l-green-500/40 text-muted-foreground" : "border-l-accent/20 text-foreground/90"
              )}
            >
        ...
        <div className="space-y-0.5">
          {habits.map((h, i) => (
            <div key={i} className="flex items-center gap-2 font-mono text-xs">
              ...
              />
              <span className={cn("text-[11px]", h.done ? "text-muted-foreground line-through" : "text-foreground/90")}>
                {h.name}
              </span>
              {h.streak > 0 && (
