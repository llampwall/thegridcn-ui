    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 space-y-0">
        {todayEvents.map((event, i) => (
          <div key={i} className="flex items-center gap-1.5 py-[3px] font-mono text-[11px] leading-tight">
            <div className={cn("h-1.5 w-1.5 shrink-0 rounded-full", categoryColors[event.category])} />
            <span className="w-9 shrink-0 text-[10px] text-muted-foreground">{event.time}</span>
        ...
        <div className="my-1 flex items-center gap-2">
          ...
          <span className="font-mono text-[8px] tracking-wider text-muted-foreground">TOMORROW</span>
          <span className="h-px flex-1 bg-border/30" />
        </div>
      ...
      {/* Footer */}
      <div className="mt-auto flex items-center gap-1 pt-1.5 font-mono text-[9px]">
        <Clock className="size-2.5 text-green-400" />
        <span className="text-green-400">2 free hours</span>
        <span className="ml-auto text-muted-foreground">+4 more</span>
      </div>
    </div>
  )
}
